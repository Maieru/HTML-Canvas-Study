...
function checkElementColision(elementMoving) {
    ...
    formasDentroDoCanvas.forEach(element => {
        if (element.id != elementMoving.id &&
            elementMoving.x.absolute + elementMoving.width.absolute >= element.x.absolute &&
            elementMoving.x.absolute <= element.x.absolute + element.width.absolute &&
            elementMoving.y.absolute + elementMoving.height.absolute >= element.y.absolute &&
            elementMoving.y.absolute <= element.y.absolute + element.height.absolute)
            aux = true;
    });
...
function eraseAllElements(context){
    formasDentroDoCanvas.forEach(element => {
        element.eraseObject(context);
    });
}
...
function scaleMouseMovement(movement){
    movement.X /= zoomX;
    movement.Y /= zoomY;
}